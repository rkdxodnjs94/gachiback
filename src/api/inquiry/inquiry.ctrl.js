...
/**
 * POST http://localhost:4001/api/inquiry
{
  title : String,
  date : String,
  content : String,
...
export const list = async (context) => {
  try {
    ...
    const inquiry = await Inquiry.find().exec();
    context.body = inquiry;
  } catch (e) {
    context.throw(500, e);
  }
...
export const remove = async (context) => {
  const { id } = context.params;
  try {
    await Inquiry.findByIdAndDelete(id).exec();
...
export const update = async (context) => {
  ...
  try {
     ...
     * 입니다.
     */
    const inquiry = await Inquiry.findByIdAndUpdate(id, context.request.body, {
      /**
       * 아래의 옵션이,
